...
array( 'db' => '`invoice`.`is_paid` AS `is_paid`', 'dt' => 6, 'field' => 'is_paid' ),
    ...
    // print_r($columns);die;
		...
		// }
			...
			// 	}
        $output_arr['data'][$key][$key_id] .= '<a sty id="btnEditRow" class="modalButton mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="'.lang('edit').'"><i class="material-icons">mode_edit</i></a>';
				$output_arr['data'][$key][$key_id] .= '<a sty id="btnDownloadPdf" class="mClass download_pdf"  href="'.base_url('invoice/get_pdf?id='.$id).'" type="button" target="_blank" data-src="'.$id.'" title="'.'PDF Download'.'"><i class="fa fa-file-pdf-o" style="font-size:24px"></i></a>';
			}
