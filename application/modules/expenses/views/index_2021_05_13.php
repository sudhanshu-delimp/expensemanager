						...
						<div class="header">
                           <h2><?php echo lang('expenses') ?></h2>
                              <ul class="header-dropdown">
                                 ...
                                 <button type="button" class="btn-sm  btn btn-primary waves-effect amDisable modalButton" data-src="" data-width="555" ><i class="material-icons">add</i><?php echo lang('add_expenses') ?></button>
                              <?php }?>
                              </ul>
                            </div>
                 	 <!-- /.box-header -->
...
<table id="example_expenses" class="table table-bordered table-striped table-hover delSelTable example_expenses">
								  <thead>
								 	<tr>
										<th>
	...
	<div class="modal-dialog">
		<div class="modal-content" >
			<div class="modal-header">
				  ...
				  <h4 class="box-title"><?php echo lang('expenses'); ?></h4>
			</div>
...
var table = $("#example_expenses").DataTable({
	...
	$('body').on('submit', '#form', function(ev) {
		ev.preventDefault();
		$('#nameModal_expenses').find('input[name="save"]').after('<span class="mka-loading"><img src="<?php echo mka_base().'assets/images/widget-loader-lg.gif'; ?>" alt="" /><span>');
		var formData = new FormData($(this)[0]);
		$.ajax({
			url: '<?php echo base_url().'expenses/add_edit' ?>',
			method: 'POST',
			async: false,
			data: formData,
			cache: false,
			...
			if(mka > 0) {
				...
				setTimeout(function() {
					if($('.submodule-main-div').length > 0) {
				        ...
				        $('#example_expenses').DataTable().ajax.reload();
...
$(document).ready(function(){
	...
	setTimeout(function() {
		...
		if(cjhk == 1) {
			$('.dataTables_info').before('<button data-del-url="<?php echo base_url() ?>expenses/delete/" rel="delSelTable" class="btn btn-default btn-sm delSelected pull-left"> <i class="material-icons col-red">delete</i> </button><br><br>');	
		}
	}, 300);
