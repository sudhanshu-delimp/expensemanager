...
class User extends CI_Controller {
    ...
    public function dashboard() {
        ...
        $user_condtion = '';
        if($user_type!='admin')
            $user_condtion = 'Where user_id = '.$this->user_id;

		$qr = " SELECT SUM(`income_amount`) AS `mka` FROM `income`   ".$user_condtion;

		$data['Total_Income_data'] = $this->User_model->getQrResult($qr);

		$qr = " SELECT SUM(`expenses_amount`) AS `mka` FROM `expenses`   ".$user_condtion;
		$data['Total_Expense_data'] = $this->User_model->getQrResult($qr);

		$qr = " SELECT SUM(`expenses_amount`) AS `mka` FROM `expenses`   WHERE  DATE_FORMAT(`expenses`.`expenses_date`, '%Y-%m-%d') = '".date('Y-m-d')."'  ";
         if($user_type!='admin')
            $qr .= 'AND user_id = '.$this->user_id;
		$data['Expense_Today_data'] = $this->User_model->getQrResult($qr);
		/* mka_info_dashboard */


					/* list Box function */
					$query = "SELECT MONTH(`expenses_date`) as `months`, `expenses_amount` AS `mka_sum` FROM  `expenses`  WHERE YEAR(`expenses_date`) = '2017'  ";
					$data['Bar_1'] = $this->User_model->get_bar_chart_data($query);
			$query = "SELECT * FROM  `expenses`  LEFT JOIN  `expense_category` AS `expense_category0` ON (`expense_category0`.`expense_category_id` = `expenses`.`expenses_category`)  ORDER BY `expenses_id` DESC LIMIT 10 ";
			$data['last_expenses_list'] = $this->User_model->getQrResult($query);
    ...
    public function dataTable (){
    	...
    	$primaryKey = 'users_id';
    	$columns = array(
           array( 'db' => 'users.users_id AS users_id', 'field' => 'users_id', 'dt' => 0 ),array( 'db' => 'users.status AS status', 'field' => 'status' , 'dt' => 1 ),
					array( 'db' => 'users.name AS name', 'field' => 'name' , 'dt' => 2 ),
					array( 'db' => 'users.email AS email', 'field' => 'email' , 'dt' => 3 ),
					array( 'db' => 'users.user_type AS user_type', 'field' => 'user_type' , 'dt' => 4 ),
    ...
    public function get_modal() {
        is_login();
        $data['user_detail'] = $this->session->get_userdata()['user_details'][0];
    ...
    public function add_edit($id='') {
        ...
        if($id != '') {
            ...
            if(empty($data['visible_user_invoice'])){
              $data['visible_user_invoice'] = '';
            }
            if(empty($data['visible_user_income'])){
              $data['visible_user_income'] = '';
            }
