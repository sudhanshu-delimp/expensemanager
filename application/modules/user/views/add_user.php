  ...
  <div class="box-body">
    <div class="row">
        ...
        if($user_detail->user_type == 'admin'){
          ...
          <div class="col-md-12">
          <div class="form-group form-float">
            <h6>User can view the invoice created by following users</h6>
            <div class="form-line">
              <?php
                foreach($users as $key=>$user){
                  ?>
                  <input type="checkbox" name="visible_user_invoice[]" id="user_<?php  echo $key; ?>"  value="<?php echo $user->users_id; ?>" <?php echo (isset($userData->visible_user_invoice) && in_array($user->users_id, explode(",", $userData->visible_user_invoice)))?'checked':''; ?> />
                  <label for="user_<?php  echo $key; ?>"><?php echo $user->name; ?></label>
                  <?php
                }
              ?>
            </div>
        ...
        <div class="col-md-12">
        <div class="form-group form-float">
          <h6>User can view the income created by following users</h6>
          <div class="form-line">
            <?php
              foreach($users as $key=>$user){
                ?>
                <input type="checkbox" name="visible_user_income[]" id="user_income_<?php  echo $key; ?>"  value="<?php echo $user->users_id; ?>" <?php echo (isset($userData->visible_user_income) && in_array($user->users_id, explode(",", $userData->visible_user_income)))?'checked':''; ?> />
                <label for="user_income_<?php  echo $key; ?>"><?php echo $user->name; ?></label>
                <?php
              }
            ?>
          </div>
          ...
          <div class="col-md-12">
            <div class="form-group imsize">
              ...
              <div class="pic_size" id="image-holder">
                ...
                <left> <img class="thumb-image setpropileam" src="<?php echo mka_base();?>/assets/images/<?php echo isset($profile_pic)?$profile_pic:'user.png';?>" alt="User profile picture"></left>
              </div> <input type="file" name="profile_pic" id="exampleInputFile">
            </div>
          </div>
        </div>
        <?php get_custom_fields('user', isset($userData->users_id)?$userData->users_id:NULL); ?>
