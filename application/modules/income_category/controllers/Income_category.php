...
class Income_category extends CI_Controller {
  	function __construct() {
	    ...
	    $this->lang->load(strtolower('Income_category'), 'english');
		if(true==1){
			is_login(); 
      ...
      */
  	public function index() {   
  		if(CheckPermission("income_category", "all_read,own_read")){
			...
			$data["view_data"]= $this->Income_category_model->get_data($con);
			if($con == '') {
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			if(isset($data['mkacf'])) {
                ...
                if(!empty($custom_fields)) {
                    foreach ($custom_fields as $cfkey => $cfvalue) {
                        ...
                        $cf_data = $this->Income_category_model->getQrResult($qr);
                        if(is_array($cf_data) && !empty($cf_data)) {
                            $d = array(
                                        "value" => $custom_fields[$cf_data[0]->cf_id],
                                    );
                            $this->Income_category_model->updateRow('cf_values', 'cf_values_id', $cf_data[0]->cf_values_id, $d);
                        } else {
                            $d = array(
                                    "rel_crud_id" => $this->input->post('id'),
                                    "cf_id" => $cfkey,
                                    "curd" => 'income_category',
                                    "value" => $cfvalue,
                                );
                            $this->Income_category_model->insertRow('cf_values', $d);
                        }
                    }
                }
            }
			...
			$last_id = $this->Income_category_model->insertRow('income_category', $data);
			if(!empty($custom_fields)) {
                foreach ($custom_fields as $cfkey => $cfvalue) {
                	if(is_array($cfvalue)) {
                		$cfvalue = implode(',', $cfvalue);
	...
	/**
      ...
      */
  	public function ajx_data(){
		...
		$joinQuery  =   "FROM `income_category` ";
		$columns 	= array(
array( 'db' => 'income_category.income_category_id AS income_category_id', 'field' => 'income_category_id', 'dt' => 0 ),
array( 'db' => 'income_category.income_category_category_name AS income_category_category_name', 'field' => 'income_category_category_name', 'dt' => 1 ),
);

			error_reporting(E_ALL);
			ini_set('display_errors', 1);
	        $cf = get_cf('income_category');
	        if(is_array($cf) && !empty($cf)) {
	            foreach ($cf as $cfkey => $cfvalue) {
	                ...
	                $joinQuery  .=  " LEFT JOIN `cf_values` AS cf_values_".$cfkey."  ON  `income_category`.`income_category_id` = `cf_values_".$cfkey."`.`rel_crud_id` AND `cf_values_".$cfkey."`.`cf_id` =  '".$cfvalue->custom_fields_id."' ";
	            }
	        }
			array_push($columns, array( 'db' => 'income_category.income_category_id AS income_category_id', 'field' => 'income_category_id', 'dt' => count($columns) ));
		$where = '';
		$j = 0;
		...
		$recordsTotal = $this->db->select("count('income_category') AS c")->get('income_category')->row()->c;
