		...
		<!-- Main content -->
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
						<div class="header">
                           <h2><?php echo lang('income_category') ?></h2>
                              <ul class="header-dropdown">
                                 <?php if(CheckPermission("income_category", "own_create")){ ?>
                                 <button type="button" class="btn-sm  btn btn-primary waves-effect amDisable modalButton" data-src="" data-width="555" ><i class="material-icons">add</i><?php echo lang('add_income_category') ?></button>
                              <?php }?>
                              </ul>
                            </div>
                 	 <!-- /.box-header -->
                     <div class="body table-responsive">
<table id="example_income_category" class="table table-bordered table-striped table-hover delSelTable example_income_category">
								  <thead>
								 	<tr>
										<th>
	...
	<div class="modal-dialog">
		<div class="modal-content" >
			<div class="modal-header">
				  ...
				  <h4 class="box-title"><?php echo lang('income_category'); ?></h4>
			</div>
...
var table = $("#example_income_category").DataTable({
        ...
        "serverSide": true,
        "ajax": {
        	"url" : url + "income_category/ajx_data",
	...
	$('body').on('submit', '#form', function(ev) {
		ev.preventDefault();
		$('#nameModal_income_category').find('input[name="save"]').after('<span class="mka-loading"><img src="<?php echo mka_base().'assets/images/widget-loader-lg.gif'; ?>" alt="" /><span>');
		var formData = new FormData($(this)[0]);
		$.ajax({
			url: '<?php echo base_url().'income_category/add_edit' ?>',
			method: 'POST',
			async: false,
			data: formData,
			cache: false,
			...
			if(mka > 0) {
				$('#nameModal_income_category').modal('hide');
				setTimeout(function() {
					if($('.submodule-main-div').length > 0) {
				        ...
				        $('#example_income_category').DataTable().ajax.reload();
...
$(document).ready(function(){
	...
	setTimeout(function() {
		...
		if(cjhk == 1) {
			$('.dataTables_info').before('<button data-del-url="<?php echo base_url() ?>income_category/delete/" rel="delSelTable" class="btn btn-default btn-sm delSelected pull-left"> <i class="material-icons col-red">delete</i> </button><br><br>');	
		}
	}, 300);
	...
	$("body").on("click",".modalButton", function(e) {  
		...
		$("#nameModal_income_category").find(".modal-body").html(loading);
		$("#nameModal_income_category").find(".modal-body").attr("style","text-align: center");    
		$.ajax({
			...
			$("#nameModal_income_category").find(".modal-body").removeAttr("style");  
