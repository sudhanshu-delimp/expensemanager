...
**/
 function CheckPermission($moduleName="", $method=""){
	...
	function selectBoxDynamic($field_name='', $tableName='setting',$colom='value',$selected='',$attr='',$whereCol='',$whereVal='',$ordercal='',$orderby='')
	{   
		$CI = get_instance();
		$CI->db->select('*');
		$CI->db->from($tableName);
		...
		if(!empty($orderby)){
		   $CI->db->order_by($ordercal, $orderby); 
		}
		$query = $CI->db->get();
				...
				if($rvalue->type == 'text' || $rvalue->type == 'numbers' || $rvalue->type == 'date' || $rvalue->type == 'email') {
					...
					if(is_array($opt) && !empty($opt)) {
						foreach ($opt as $opkey => $opvalue) {
							...
							$html .= '<option value="'.$opvalue.'" '.$selected.' >'.ucfirst($opvalue).'</option>';
						}
					}
					$html .= '</select>';
					//$html .= '<label class="form-label">'.ucfirst(lang(get_lang($crud).'_'.get_lang($rvalue->name))).' '.$ls.'</label>';
					$html .= '<label class="form-label">'.ucfirst($rvalue->name).' '.$ls.'</label>';
					$html .= '</div>';
