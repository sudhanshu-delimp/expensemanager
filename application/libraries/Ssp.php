...
class SSP {
    ...
    static function mka_data_output ( $columns, $data, $isJoin = false )
    {
        $out = array();
    ...
    static function mkaorder ( $request, $columns, $isJoin = false )
    {
        $order = '';
            ...
            for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['orderable'] == 'true' ) {
                    $dir = $request['order'][$i]['dir'] === 'asc' ?
                        'ASC' :
                        'DESC';
                    if(strpos($column['db'], 'AS') > 0) {
                        $col = explode('AS', $column['db']);
                        $orderBy[] = trim($col[0]).' '.$dir;
                    } else {
    ...
    static function mkaFilter ( $request, $columns, $isJoin = false )
    {
        $globalSearch = array();
        $columnSearch = array();
        $dtColumns = SSP::pluck( $columns, 'dt' );
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    if($isJoin && strpos($column['db'], 'AS') > 0) {
                        $col = explode('AS', $column['db']);
                        $globalSearch[] = trim($col[0])." LIKE '%".$str."%' ";
                    } else {
                        $col = explode('AS', $column['db']);
                        $globalSearch[] = trim($col[0])." LIKE '%".$str."%' ";
                    }
